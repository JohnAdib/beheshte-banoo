...
class users 
{
	...
	{
		$this->form("text")->name("logincounter")->min(0)->max(99)->required()->type('number');
	}
	...
	{
		$this->form("text")->name("feedback")->min(0)->max(99)->type('number');
	}
	...
	{
		$this->form("select")->name("booth_")->min(0)->max(99)->type("select")->validate()->id();
		$this->setChild();
	}
}
?>